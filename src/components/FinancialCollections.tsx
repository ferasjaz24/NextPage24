      ...
      <html dir="rtl">
        <head>
          ...
          <style>
            ...
            .header-sec h1 { color: #1C5B6B; font-size: 24px; margin-bottom: 10px; }
            ...
            .stat-card p { color: #1C5B6B; font-size: 20px; font-weight: bold; margin: 0; }
        ...
        <body>
          <div style="text-align: center;"><img src="https://i.postimg.cc/0jQj3XVc/black-logo-nextpage-8.png" referrerpolicy="no-referrer" alt="NextPage Co Logo" style="width: 120px; height: 120px; object-fit: contain; margin-bottom: 10px;" /></div>
    ...
    <div className="space-y-6" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
          <DollarSign className="w-6 h-6 text-[#1C5B6B]" />
          قسم التحصيل المالي
        </h2>
        <div className="flex gap-2">
          ...
          <button 
            ...
            className="bg-[#1C5B6B] hover:bg-sky-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition"
          >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input 
            ...
            className="w-full pl-4 pr-10 py-2 border rounded-xl font-bold text-sm bg-slate-50 focus:bg-white focus:border-[#1C5B6B] focus:ring-1 focus:ring-[#1C5B6B] transition"
          />
        </div>
        <select 
          value={statusFilter}
          onChange={e => setStatusFilter(e.target.value)}
          className="w-full md:w-64 p-2 border rounded-xl font-bold text-sm bg-slate-50 focus:bg-white focus:border-[#1C5B6B]"
        >
        ...
        <input 
          ...
          className="w-full md:w-48 p-2 border rounded-xl font-bold text-sm bg-slate-50 focus:bg-white focus:border-[#1C5B6B]"
        />
        <select 
          value={sortOrder}
      ...
      {loading ? (
        <div className="flex justify-center p-12">
          <div className="w-8 h-8 border-4 border-[#1C5B6B] border-t-transparent rounded-full animate-spin"></div>
        </div>
            ...
            <div key={plan.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden relative">
               <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div className="flex items-start gap-3">
                    <button onClick={() => togglePlan(plan.id)} className="text-[#1C5B6B] hover:bg-blue-100 p-1 rounded-full transition mt-1">
                      {expandedPlans.includes(plan.id) ? <ChevronUp className="w-6 h-6" /> : <ChevronDown className="w-6 h-6" />}
                    </button>
                    <div>
                      <h3 className="text-lg font-black text-[#1C5B6B] mb-1 flex items-center gap-2 flex-wrap">
                   ...
                   <tbody className="divide-y divide-slate-50">
                     {plan.phases.map(ph => (
                       <tr key={ph.id} className="hover:bg-slate-50">
                         ...
                         <td className="p-3 font-bold text-[#1C5B6B]">{ph.percentage}%</td>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-start justify-center p-4 overflow-y-auto pt-10 pb-24">
          <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl flex flex-col min-h-[500px]">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50 rounded-t-3xl sticky top-0 z-40">
              <h2 className="text-xl font-bold text-[#1C5B6B] flex items-center gap-2">
            ...
            <div className="p-6 flex-1 space-y-6">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className={`relative group/quote z-20 ${isReadOnlyModal ? 'pointer-events-none opacity-80' : ''}`} tabIndex={isReadOnlyModal ? -1 : 0}>
                   ...
                   <div className="w-full p-3 border rounded-xl font-bold bg-slate-50 focus-within:bg-white focus-within:border-[#1C5B6B] flex justify-between items-center cursor-pointer">
                   ...
                   <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 shadow-xl rounded-xl p-2 hidden group-focus-within/quote:block hover:block z-30 max-h-60 overflow-y-auto">
                     <input
                       ...
                       className="w-full p-2 mb-2 border rounded-xl font-bold bg-slate-50 focus:bg-white focus:border-[#1C5B6B] text-sm sticky top-0"
                     />
                 ...
                 <div>
                   ...
                   <select 
                     ...
                     className="w-full p-3 border rounded-xl font-bold bg-slate-50 focus:bg-white focus:border-[#1C5B6B] disabled:opacity-50"
                   >
               ...
               {selectedQuoteId && paymentSystem && draftPhases.length > 0 && (
                 <div className="border border-slate-200 rounded-2xl overflow-hidden">
                   <div className="bg-[#1C5B6B]/10 p-3 text-[#1C5B6B] font-bold text-sm flex justify-between items-center">
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 rounded-b-3xl flex justify-end gap-3">
              ...
              <button 
                ...
                className={`px-6 py-2.5 rounded-xl font-bold text-white bg-[#1C5B6B] hover:bg-sky-700 transition flex items-center gap-2 ${isReadOnlyModal ? 'hidden' : ''}`}
              >
