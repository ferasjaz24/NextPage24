...
الطرف الثاني: هو شركة الصفحة التالية للخدمات التقنية.
...
- يبدأ سريان الضمان من تاريخ التركيب، بشرط وجود ختم شركة الصفحة التالية للخدمات التقنية والمكون من (شعار ورقم الهاتف) على اللوحة.`;
  ...
  const generateSingleHTML = (sq, matchingClient, totalRounded, subtotal, vat, total, formattedTermsText) => {
    ...
    return `
            <div class="quotation-print-page">
              ...
              <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #1C5B6B; padding-bottom: 8px; margin-bottom: 20px; user-select: none; direction: ltr;">
                <!-- معلومات الشركة -->
        ...
        <div className="space-y-6">
           <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-slate-100 gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={handleCreateNew} className="flex items-center gap-2 px-6 py-3 bg-[#1C5B6B] hover:bg-sky-700 text-white rounded-xl text-sm font-black shadow-lg transition">
           ...
           <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <input type="text" value={searchName} onChange={e=>setSearchName(e.target.value)} className="w-full p-2.5 border rounded-xl text-sm outline-none focus:border-[#1C5B6B]" placeholder="..." />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">رقم الكوتيشن:</label>
                <input type="text" value={searchQtn} onChange={e=>setSearchQtn(e.target.value)} className="w-full p-2.5 border rounded-xl text-sm outline-none focus:border-[#1C5B6B]" placeholder="QTN..." dir="ltr" style={{textAlign:'right'}} />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">الترتيب:</label>
                <select value={sortOrder} onChange={e=>setSortOrder(e.target.value as any)} className="w-full p-2.5 border rounded-xl text-sm outline-none focus:border-[#1C5B6B]">
              ...
              <div>
                ...
                <select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)} className="w-full p-2.5 border rounded-xl text-sm outline-none focus:border-[#1C5B6B]">
                  ...
                  {filteredQuotes.map(q => {
                    ...
                    return (
                      <tr key={q.id} className={`transition ${isSelected ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}>
                        ...
                        <td className="p-4 font-mono font-bold text-[#1C5B6B]" dir="ltr" style={{textAlign:'right'}}>{q.quotationNumber || '---'}</td>
    ...
    <div className="bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden relative pb-20">
       <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between sticky top-0 z-10">
         ...
         <div className="flex gap-2">
            ...
            <button onClick={onPreview} className="flex items-center gap-2 px-4 py-2 bg-[#1C5B6B] hover:bg-sky-700 text-white rounded-xl text-xs font-bold transition"><Eye className="w-4 h-4"/> معاينة طباعة</button>
       ...
       <div className="flex gap-4 p-4 border-b border-slate-100 text-sm font-bold bg-white">
         <button onClick={()=>setTab('basic')} className={`px-6 py-2.5 rounded-full transition ${tab==='basic' ? 'bg-[#1C5B6B] text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}>المعلومات الأساسية</button>
         <button onClick={()=>setTab('print')} className={`px-6 py-2.5 rounded-full transition ${tab==='print' ? 'bg-[#1C5B6B] text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}>معلومات الطباعة والشروط</button>
       </div>
         ...
         <div className="p-6 space-y-8 animate-fade-in relative ${isReadOnly ? 'opacity-90 pointer-events-none' : ''}">
           ...
           <div className="space-y-6">
              ...
              <div className="bg-slate-50 p-4 border rounded-2xl w-full relative">
                ...
                <input 
                  ...
                  className="w-full p-2.5 border rounded-xl text-sm font-bold bg-white focus:outline-none focus:border-[#1C5B6B]"
           ...
           <div>
              ...
              <div className="border border-slate-200 rounded-2xl overflow-hidden overflow-x-auto shadow-sm">
                <table className="w-full text-sm text-right">
                  <thead className="bg-[#1C5B6B] text-white">
                    <tr>
                      ...
                      <tr key={it.id}>
                        ...
                        <td className="p-3 relative">
                           ...
                           <input 
                             ...
                             className="w-full p-2 border rounded-lg font-mono text-xs focus:border-[#1C5B6B] outline-none" 
           ...
           <div className="flex justify-end mt-4">
              <div className="w-full md:w-1/3 bg-slate-50 p-5 rounded-2xl border border-slate-200">
                 <h4 className="font-black text-sm mb-4 text-[#1C5B6B] border-b pb-2">المجاميع</h4>
       ...
       {editingItemId && itemInEdit && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm" onMouseDown={(e)=>{if(e.target===e.currentTarget) setEditingItemId(null)}}>
             <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden p-6 space-y-5" onMouseDown={e=>e.stopPropagation()}>
                <h3 className="font-black text-lg text-[#1C5B6B] border-b pb-3">معلومات إضافية للصنف رقم # {quote.items.findIndex((i:any)=>i.id===editingItemId)+1}</h3>
                ...
                <div className="pt-4 flex justify-end">
                  <button onClick={()=>setEditingItemId(null)} className="px-6 py-2.5 bg-[#1C5B6B] text-white rounded-xl font-bold shadow-lg shadow-sky-200">حفظ وحفظ التعديلات</button>
                </div>
             </div>
          </div>
       )}
