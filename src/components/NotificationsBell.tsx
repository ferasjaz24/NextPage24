  ...
  return (
    <div className="relative" ref={containerRef}>
      <button 
        ...
        className="relative p-2 bg-slate-200/60 hover:bg-[#1C5B6B] hover:text-white text-slate-600 rounded-xl transition-colors outline-none"
      >
        ...
        <div className={`absolute top-full mt-2 w-80 bg-white border border-slate-200 shadow-2xl rounded-2xl z-50 overflow-hidden ${lang === 'ar' ? 'left-0' : 'right-0'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
          <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
            <h3 className="font-bold text-slate-800 flex items-center gap-2">
              <Bell className="w-4 h-4 text-[#1C5B6B]" />
              {lang === 'ar' ? 'الإشعارات' : 'Notifications'}
            </h3>
            {unreadCount > 0 && (
              <span className="text-xs text-[#1C5B6B] font-bold bg-blue-100 px-2 py-1 rounded-full">
          ...
          <div className="p-3 border-t border-slate-100 bg-slate-50 text-center">
            <button className="text-xs font-bold text-[#1C5B6B] hover:underline">
