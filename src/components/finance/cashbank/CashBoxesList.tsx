      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center">
        <div className="relative w-full sm:w-96">
          ...
          <input
            ...
            className="w-full pl-4 pr-10 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1C5B6B]"
            dir={lang === "ar" ? "rtl" : "ltr"}
          />
        </div>
        <button
          ...
          className="flex items-center gap-2 bg-[#1C5B6B] text-white px-4 py-2 rounded-lg hover:bg-[#005a96] transition-colors whitespace-nowrap"
        >
          ...
          <tbody className="divide-y divide-slate-200">
            {filteredBoxes.map((box) => (
              <tr key={box.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3 text-right font-bold text-[#1C5B6B]">{box.current_balance.toLocaleString()}</td>
