...
const DocumentHeader = () => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #1C5B6B', paddingBottom: '16px', marginBottom: '32px', userSelect: 'none', direction: 'ltr' }}>
    ...
    <div style={{ textAlign: 'right' }}>
      <img src="https://i.postimg.cc/0jQj3XVc/black-logo-nextpage-8.png" referrerPolicy="no-referrer" alt="NextPage Co Logo" style={{ width: '120px', height: '120px', objectFit: 'contain' }} />
    </div>
  </div>
);

const DocumentFooter = () => (
  <div style={{ marginTop: 'auto', borderTop: '1px solid #1C5B6B', paddingTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', fontSize: '10px', color: '#4b5563', userSelect: 'none', direction: 'ltr', minHeight: '80px' }}>
    <div style={{ textAlign: 'left', lineHeight: '1.6' }}>
      <p style={{margin:0}}><span style={{ fontWeight: 'bold', color: '#1C5B6B' }}>T:</span> +966 13 833 4115</p>
      <p style={{margin:0}}><span style={{ fontWeight: 'bold', color: '#1C5B6B' }}>Factory:</span> Dallah Industrial District, Dammam 32445, Saudi Arabia.</p>
    </div>
    <div style={{ textAlign: 'right', lineHeight: '1.6' }}>
      <p style={{margin:0}}>info@nextpage.co | www.nextpage.co</p>
      <p style={{margin:0}}><span style={{ fontWeight: 'bold', color: '#1C5B6B' }}>Riyad Bank Iban:</span> SA6 320 000 003 220 402 999 901</p>
    </div>
  </div>
);
  ...
  const getContractLegalBody = () => {
    ...
    if (selectedTemplate === 'expat_muqeem') {
      return {
        ...
        body_ar: `إنه في يوم ${today}، جرى الاتفاق والتعاقد بين شركة الصفحة التالية للخدمات التقنية وبصفتها الطرف الأول، والسيد ${empName} (جنسيته وافد وجوازه رقم ${activeEmp.passportDetails} وإقامته رقم ${activeEmp.iqamaId}) بصفته الطرف الثاني على التالي:
...
3. This agreement is tied sequentially to Iqama work authorization validities. Changes or renewals shall be synchronized immediately on Ministry of Labor Qiwa channels.`
    ...
    if (selectedTemplate === 'temp_project') {
      return {
        ...
        body_ar: `عقد عمل مخصص لإنجاز مهام وعقود تركيب لوحات مشروع فني ("موقت - مول شابيلي جراند بالرياض") بين الطرف الأول شركة الصفحة التالية للخدمات التقنية والطرف الثاني السيد ${empName}:
...
3. Compensation shall be restricted to SAR ${basicPay} plus designated field bonuses.`
    ...
    return {
      ...
      body_ar: `إنه في يوم ${today}، جرى تحرير هذا الاتفاق بالتراضي بين شركة الصفحة التالية للخدمات التقنية (الطرف الأول) والمواطن السعودي السيد ${empName} (هوية وطنية رقم ${activeEmp.iqamaId}) ويشار إليه بـ (الطرف الثاني) على ما يلي:
  ...
  return (
    <div id="hr-contracts-module" className="bg-white/60 backdrop-blur-md p-6 rounded-3xl border border-white/50 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 border-b border-slate-100 gap-4">
        <div>
          <h4 className="text-sm font-black text-[#1C5B6B] flex items-center gap-2">
            <FileText className="w-4.5 h-4.5 text-[#229FA0]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-4 bg-slate-50/60 p-4 rounded-2xl border border-slate-100 space-y-4 text-xs font-semibold">
          ...
          <form onSubmit={handleAddAmendment} className="pt-3 border-t border-slate-200 space-y-2">
            <label className="block text-[#1C5B6B] font-black">{lang === 'ar' ? '✍️ صياغة ملاحاق وشروط خاصة إضافية:' : '✍️ Append Custom Addendum Clauses:'}</label>
            <textarea 
            ...
            <button 
              type="submit" 
              className="w-full py-1.5 bg-[#1C5B6B] hover:bg-[#1C5B6B]/95 text-white rounded-xl text-[10px] font-black"
            >
        ...
        <div id="contracts-document-preview-pane" className="md:col-span-8 bg-white border border-slate-205 p-8 rounded-2xl shadow-sm space-y-6 relative print:p-0 print:border-0 print:shadow-none">
          ...
          <div className="space-y-4 leading-relaxed text-slate-800">
            <div className="text-center font-black text-sm text-[#1C5B6B] border-b pb-2">
          ...
          <div className="pt-8 border-t border-dashed grid grid-cols-2 gap-4 text-xs font-bold text-slate-700 text-center mb-8">
            <div className="space-y-4">
              <p>{lang === 'ar' ? 'الطرف الأول: شركة الصفحة التالية للخدمات التقنية' : 'First Party: NextPage Co Director'}</p>
