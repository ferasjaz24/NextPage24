...
export default function HrRecruitmentTab({ lang, onAddEmployeeLocally }: HrRecruitmentTabProps) {
  ...
  return (
    <div id="hr-recruitment-module" className="bg-white/60 backdrop-blur-md p-6 rounded-3xl border border-white/50 space-y-6">
      ...
      <h4 className="text-sm font-black text-[#1C5B6B] flex items-center justify-between pb-2 border-b border-slate-100">
        <span className="flex items-center gap-2">
          <Target className="w-4.5 h-4.5 text-[#229FA0]" />
          {lang === 'ar' ? '🎯 التوظيف الذكي ومخطط المقابلات الفنية' : 'Recruitment pipeline & Candidate screening rubric'}
        </span>
        <button 
          onClick={() => setIsReqOpen(true)}
          className="px-3 py-1 bg-[#1C5B6B] text-white text-[10px] font-black rounded-xl hover:bg-[#1C5B6B]/90"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {requisitions.map(req => (
          <div key={req.id} className="p-4 bg-slate-50/75 rounded-2xl border border-slate-100 text-xs space-y-1 relative">
            ...
            <p className="text-[10px] text-[#1C5B6B] font-mono">{req.dept}</p>
            <p className="text-[11px] text-slate-500 italic mt-1 font-mono">"Reason: {req.reason}"</p>
            <p className="text-[10px] text-slate-400 border-t pt-1 mt-1">Budget Cap: <strong className="font-mono text-[#1C5B6B]">SAR {req.budgetCap}/mo</strong></p>
          </div>
        ))}
      </div>
              ...
              return (
                <button
                  ...
                  className={`w-full text-right p-3 rounded-xl flex items-center justify-between transition-all text-xs ${
                    isSelected ? 'bg-[#1C5B6B] text-white font-bold' : 'bg-white text-slate-700 hover:bg-slate-105'
                  }`}
                >
                  <div>
        ...
        {activeCandidate && (
          <div className="lg:col-span-8 bg-white border border-slate-150 p-5 rounded-2xl space-y-4">
            ...
            <div className="flex justify-between items-start pb-3 border-b flex-wrap gap-2">
              ...
              {activeCandidate.status !== 'PROMOTED_TO_EMP' ? (
                <button
                  ...
                  className="px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-[#229FA0] text-white hover:brightness-105 shadow-md flex items-center gap-1.5 rounded-xl text-xs font-black animate-bounce"
                >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {activeCandidate.skills.map((s, idx) => (
                  <span key={idx} className="px-2 py-0.5 bg-sky-50 text-[#1C5B6B] text-[10px] font-bold rounded-lg border border-sky-100">
                    ⚙️ {s}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-slate-50 p-4 rounded-xl space-y-3">
              <span className="text-[10px] uppercase font-black text-[#1C5B6B] block">⭐ Technical & Visual Interview Rating Rubric:</span>
            ...
            <div className="text-xs text-slate-400 flex justify-between border-t border-slate-100/60 pt-2 font-mono">
              ...
              <span>Requisition Cap: <strong className="text-[#1C5B6B]">SAR 7,500/mo</strong></span>
            </div>
