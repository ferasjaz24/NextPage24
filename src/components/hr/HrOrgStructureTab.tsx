...
export default function HrOrgStructureTab({ lang, employees }: HrOrgStructureTabProps) {
  ...
  const [nodes, setNodes] = useState<OrgNode[]>([
    { id: '1', name_ar: 'الشركة الأم: الصفحة التالية للدعاية والنيون', name_en: 'Al-Waleed Signage & Neon Co. Ltd.', parentId: null, managerName_ar: 'فراس (المدير الإداري)', managerName_en: 'Feras (MD & Owner)', annualBudget: 4200000, vacanciesCount: 0 },
      ...
      <div className="flex justify-between items-center pb-2 border-b border-slate-100">
        <div>
          <h4 className="text-sm font-black text-[#1C5B6B] flex items-center gap-2">
            <Layers className="w-4 h-4 text-[#229FA0]" />
        ...
        <button 
          ...
          className="px-3 py-1.5 bg-[#1C5B6B] hover:bg-[#1C5B6B]/95 text-white text-[11px] font-black rounded-xl flex items-center gap-1"
        >
        ...
        <div className="p-4 bg-rose-50 border border-rose-200 text-rose-700 rounded-2xl flex items-start gap-2 text-xs">
          ...
          <div className="space-y-1">
            ...
            <button 
              ...
              className="text-[#1C5B6B] underline font-black block mt-1"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
              ...
              return (
                <div 
                  ...
                  className={`flex items-center justify-between p-2 rounded-xl text-xs transition-all ${indentClass} ${
                    isSelected ? 'bg-[#1C5B6B] text-white shadow-md font-bold' : 'bg-white text-slate-700 hover:bg-slate-100/60'
                  }`}
                >
                  <button 
                  ...
                  <span className={`px-1.5 py-0.5 rounded text-[8px] font-mono mr-2 ${isSelected ? 'bg-white/20 text-white' : 'bg-[#1C5B6B]/10 text-[#1C5B6B]'}`}>
        ...
        <div className="md:col-span-5 bg-white border border-slate-150 p-5 rounded-2xl space-y-4">
          <div className="border-b pb-3 space-y-1">
            <span className="text-[9px] bg-[#229FA0]/20 text-[#1C5B6B] font-black px-2 py-0.5 rounded uppercase">{lang === 'ar' ? 'التفاصيل والتبعية' : 'Node Properties'}</span>
          ...
          <div className="space-y-3 text-xs text-slate-650">
            ...
            <div>
              ...
              <p className="font-black text-[#1C5B6B] font-mono"><SaudiRiyal /> {activeNode.annualBudget.toLocaleString('en-US')}</p>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 space-y-2">
              ...
              <div className="flex gap-2 pt-2">
                <button 
                  ...
                  className="flex-1 py-1.5 bg-slate-100 hover:bg-[#1C5B6B]/10 text-[#1C5B6B] rounded-lg text-[10px] font-black flex items-center justify-center gap-1"
                >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <form onSubmit={handleAddNewUnit} className="bg-white p-6 rounded-3xl border border-slate-100 max-w-md w-full space-y-4">
            ...
            <div className="flex gap-2 text-xs font-black">
              <button type="submit" className="flex-1 bg-[#1C5B6B] text-white py-2 rounded-xl">
      ...
      {isEditOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <form onSubmit={handleEditUnit} className="bg-white p-6 rounded-3xl border border-slate-100 max-w-md w-full space-y-4">
            <h5 className="font-extrabold text-[#1C5B6B] text-sm">
