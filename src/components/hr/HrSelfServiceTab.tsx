    ...
    const printHtml = `
      ...
      <head>
        ...
        <style>
          ...
          .info-box { border: 2px solid #1C5B6B; padding: 15px; border-radius: 8px; margin-bottom: 25px; background: #f8fafc; }
    ...
    <div id="hr-selfservice-management" className="space-y-6">
      ...
      {!(user?.role === 'Employee (Inquiries)' || user?.role === 'Employee' || user?.role === 'موظف' || user?.role === 'موظف - استعلامات') && (
        <div id="self-service-view-toggle" className="flex bg-slate-100/90 p-1 rounded-2xl w-max gap-1 border border-slate-200/50">
          <button
            ...
            className={`px-4.5 py-2 rounded-xl text-xs font-black transition-all ${!forceViewAsEmployee ? 'bg-white text-[#1C5B6B] shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
          >
          ...
          <button
            ...
            className={`px-4.5 py-2 rounded-xl text-xs font-black transition-all ${forceViewAsEmployee ? 'bg-[#1C5B6B] text-white shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
          >
      ...
      <div className="glass-panel rounded-3xl p-6 relative overflow-hidden bg-gradient-to-r from-sky-50 to-white text-slate-800 shadow-md">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <span className="px-3 py-1 bg-[#1C5B6B]/15 text-[#1C5B6B] rounded-full text-[10px] font-bold uppercase tracking-wider">
            ...
            <h3 className="text-2xl font-black mt-2 text-[#1C5B6B]">
              {isEmployeeRole 
          ...
          <button 
            ...
            className="p-2.5 bg-sky-50 hover:bg-sky-100 text-[#1C5B6B] rounded-xl transition flex items-center gap-1 text-[11px] font-bold"
          >
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="glass-panel p-5 bg-white rounded-3xl border border-slate-100/90 shadow-sm space-y-4">
              ...
              <form onSubmit={handleSelfLeaveSubmit} className="space-y-3 text-xs">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 border border-slate-200 px-2 py-1.5 rounded-lg text-[#1C5B6B] font-mono"
                    />
                  </div>
                  <div>
                ...
                <button 
                  type="submit"
                  className="w-full py-2 bg-[#1C5B6B] text-white rounded-lg font-bold hover:bg-sky-700 transition"
                >
        ...
        <div className={isEmployeeRole ? "lg:col-span-8 flex flex-col gap-6" : "lg:col-span-12 flex flex-col gap-6"}>
          ...
          <div className="glass-panel p-6 bg-white rounded-3xl border border-slate-100/90 shadow-sm space-y-4">
            <h4 className="text-sm font-black text-slate-800 flex items-center gap-1.5">
              <ClipboardList className="w-4.5 h-4.5 text-[#1C5B6B]" />
            ...
            {resolvingId && (
              <form onSubmit={handleResolveInquiry} className="p-4 bg-slate-50 border rounded-2xl space-y-3 mb-4 text-xs">
                <p className="font-bold text-[#1C5B6B]">✍️ {lang === 'ar' ? 'الرد وإرسال رد الموارد والإنتاج:' : 'Draft Response Content:'}</p>
                  ...
                  <tbody>
                    ...
                    {filteredInquiries.map((item) => (
                      <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50/50 text-slate-600">
                        ...
                        <td className="py-3 px-2 text-center font-mono font-bold text-[#1C5B6B]">{item.dateCreated}</td>
                                ...
                                {item.hrNotes.includes('http') ? (
                                        <>
                                          ...
                                          <a 
                                            ...
                                            target="_blank" 
                                            rel="noreferrer"
                                            className="mt-1 inline-flex items-center gap-1 p-1.5 px-3 bg-[#1C5B6B] text-white rounded text-[10px] font-black hover:brightness-105"
                                          >
                        ...
                        <td className="py-3 px-2 text-center">
                          {item.status === 'PENDING' ? (
                            <button
                              ...
                              className="px-2.5 py-1.5 bg-[#1C5B6B] hover:bg-sky-700 text-white rounded-lg text-[10px] font-bold"
                            >
              ...
              <div className="space-y-6 text-right" dir="rtl">
                  ...
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-t-xl text-xs font-black transition-all ${
                      employeeActiveSection === 'payroll'
                        ? 'bg-[#1C5B6B] text-white shadow-sm font-black'
                        : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
                    }`}
                  >
                ...
                {employeeActiveSection === 'inquiries' && (
                  <div className="space-y-3" id="employee-inquiries-tab">
                    <h5 className="font-extrabold text-[#1C5B6B] text-xs pb-1 border-b flex items-center gap-1">
                        ...
                        generalInquiries.map(item => (
                          <div key={item.id} className="p-4 bg-slate-50 border border-slate-100 rounded-2xl space-y-2 hover:shadow-sm transition">
                            <div className="flex justify-between items-center text-[10px] border-b pb-1">
                              <span className="px-2 py-0.5 bg-blue-100/60 text-[#1C5B6B] rounded-full font-black">
                                {lang === 'ar' ? item.category_ar : item.category_en}
                              </span>
                ...
                {employeeActiveSection === 'payroll' && (
                  <div className="space-y-3" id="employee-payroll-tab">
                    <h5 className="font-extrabold text-[#1C5B6B] text-xs pb-1 flex items-center gap-1.5">
                        ...
                        receivedPayslips.map(item => (
                          <div key={item.id} className="p-4 bg-gradient-to-br from-emerald-50/40 to-white border border-emerald-100 rounded-3xl space-y-3 shadow-sm hover:shadow transition relative overflow-hidden text-right">
                            ...
                            <div className="flex justify-between items-center pt-2">
                              <span className="text-[9px] text-[#1C5B6B] font-mono">ID: {item.id}</span>
                              <button
                                onClick={() => handlePrintInquiryPayslip(item)}
                                className="px-3.5 py-1.5 bg-[#1C5B6B] hover:bg-[#1C5B6B]/95 text-white rounded-xl text-[10px] font-black transition flex items-center gap-1"
                              >
