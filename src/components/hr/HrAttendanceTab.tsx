      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 border-b border-slate-100 gap-4">
        <div>
          <h4 className="text-sm font-black text-[#1C5B6B] flex items-center gap-2">
            <Clock className="w-4.5 h-4.5 text-[#229FA0]" />
        ...
        <button 
          ...
          className="px-3 py-1.5 bg-[#1C5B6B] text-white hover:bg-[#1C5B6B]/90 rounded-xl text-xs font-black flex items-center gap-1.5 shadow-md"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              return (
                <div 
                  ...
                  className={`p-3 rounded-xl border flex flex-col justify-between cursor-pointer transition-all ${
                    isSelected ? 'bg-white border-[#1C5B6B] shadow-lg scale-102 font-bold' : 'bg-white/80 border-slate-100 hover:bg-white'
                  }`}
                  onClick={() => setSelectedLogId(log.id)}
                >
                  <div className="flex justify-between items-center">
                    <span className="font-extrabold text-slate-800">{log.name}</span>
                    <span className="text-[10px] font-mono font-black text-[#1C5B6B]">{log.checkIn} - {log.checkOut}</span>
                  </div>
        ...
        {activeLog && (
          <div className="lg:col-span-7 bg-white border border-slate-150 p-5 rounded-2xl space-y-4">
            <h5 className="font-extrabold text-xs text-[#1C5B6B] uppercase tracking-wide border-b pb-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs bg-slate-50 p-4 rounded-xl">
              <div>
                <span className="text-[9px] text-[#1C5B6B] font-bold block">{lang === 'ar' ? 'مستهدف إحداثيات اللوحة الإعلانية:' : 'Billboard Designated Coordinates:'}</span>
